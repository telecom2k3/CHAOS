...
- Various checks for headers that indicate a bad message.  A variety of Mailchimp sanity checks are performed.  JR\_EXCHANGE is a callout rule set when Microsoft Exchange Server headers are detected.  Many Microsoft Exchange header sanity checks are also performed.  JR\_DUP\_HDRS hits whenever multiple IDENTICAL header lines appear in a message.  There are also tests for headers that should never appear more than once.  There are also checks for headers that shouldn't appear in the presence of other headers.
