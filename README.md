...
- This is an immutable rulename that is set if there is a SendGrid
...
- Various checks for headers that indicate a bad message.  A variety of Mailchimp sanity checks are performed.  JR\_EXCHANGE is a callout rule set when Microsoft Exchange headers are detected in a message.  Many Microsoft Exchange header checks are also performed.  All rules are callout values unless marked with an asterisk (\*).  These are scored at various rates when in Auto mode.
...
- Tuorum Periculo: If an Eval rule provided in this plugin does not meet the requirements of you or your clients, please disable (comment out) the rule and report the problem.  See the BUGS section for information regarding problem reporting.
