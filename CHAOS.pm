...
sub check_email_greets {
    ...
    if (( defined $toaddr ) && ( defined $usrpart )) {
        chomp $toaddr;
...
sub mailer_check {
    ...
    } elsif ( $pmail =~ /[0-9]{2,8}:(MatrixInboxV16|leaf|gzip64)\.php/g ) {
        $count++;
        if ( $mode eq "AutoISP" ) { 
            $score = 0.3 * $pms->{conf}->{chaos_tag};
        }
