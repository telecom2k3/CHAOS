...
sub from_lookalike_unicode {
	...
	if ($code > $max) {
		if ( $mode eq "AutoISP" ) {	
				...
				$score = $pms->{conf}->{chaos_tag} * 0.18 * ($code - $max + 1);
		}
		$pms->{conf}->{descriptions}->{"$rulename"} = $description;
		$pms->got_hit("$rulename", "HEADER: ", score => $score);
		$pms->{conf}->{scoreset}->[$set]->{"$rulename"} = sprintf("%0.4f", $score); 
...
SOBIG and SOBER worms.  In Auto mode, JR_ADMIN_FRAUD is set to a high watermark
value of ${chaos_high}; one of two rules herein that score as such.
...
In Auto mode, JR_ADMIN_BODY is set to a high watermark value of ${chaos_high}; 
...
=item  This is an immutable rulename that is set if there is a SendGrid
 reference in ANY header.  A fixed score is returned in Auto mode.  In
 all other modes, the score is set to a callout value of 0.01.
...
sub check_for_sendgrid {
    my ( $self, $pms ) = @_;
    local $subject = $pms->get('ALL');
	local $sgreference = $pms->get('References', undef);
	local $sg_eid = $pms->get("X-SG-EID", undef);
    local @count = $subject =~ /\bsendgrid\.net\b/g;
	@count = grep defined, @count; 
	local $sendgrid = scalar @count;
    local $set = 0;
	local $mode = $pms->{conf}->{chaos_mode};
	local $score = 0.01;
	local $rulename = "JR_HAS_SENDGRID";
	local $description = "CHAOS: Sendgrid observed in Headers";
	if ( $mode eq "AutoISP" ) {	
			$score = 0.28 * $pms->{conf}->{chaos_tag};
	}
...
=item  This checks for the presence of a User-Agent header and tags 
...
=item  Various checks for headers that indicate a bad message.  A variety of Mailchimp sanity checks are performed.  JR_EXCHANGE is a callout rule set when Microsoft Exchange headers are detected in a message.  Many Microsoft Exchange header checks are also performed.  All rules are callout values unless marked with an asterisk (*).  These are scored at various rates when in Auto mode. 

	JR_BOGUS_HEADERS *      JR_BAD_CHIMP *
	JR_X_BEENTHERE *        JR_X_SENTBY *
	JR_EXCHANGE             JR_EXCHANGE_AUTH *
	JR_EXCHANGE_ANON *      JR_EXCH_BAD_AUTH *
	JR_EXCH_ATTACH *		
...
sub mailer_check {
    my ( $self, $pms ) = @_;
	local $swift = $pms->get('X-Gnkw-Mailer', undef);
	...
	} elsif ( $xmail =~	/Zimbra_/g ) {
		$score = 0.01;
		$count++;
		$rulename = "JR_ZIMBRA";
		$mailer = "ZIMBRA Collaboration Server";
	} elsif ( $xmail =~	/BM23_Mail/g ) {
		if ( $mode eq "AutoISP" ) {	
	...
	} elsif ( $xmail =~	/Sendy_\(https:\/\/sendy\.co\)/g ) {
		$score = 0.01;
		$count++;
		$rulename = "JR_SENDY";
	...
	if ( $allheaders =~ /cp20\.com/g ) {
		...
		$description .= "CHAOS X-Mailer is: $mailer";
		$pms->{conf}->{descriptions}->{"$rulename"} = $description;
		$pms->got_hit("$rulename", "HEADER: ", score => $score);
		$pms->{conf}->{scoreset}->[$set]->{"$rulename"} = sprintf("%0.4f", $score); 
	...
	if (( defined $mailgun1 ) || ( defined $mailgun2 )) {
		$rulename = "JR_MAILGUN";
		$description = "CHAOS: Mail sent with Mailgun";
		$pms->{conf}->{descriptions}->{"$rulename"} = $description;
		$pms->got_hit("$rulename", "HEADER: ", score => $score);
		$pms->{conf}->{scoreset}->[$set]->{"$rulename"} = sprintf("%0.4f", $score); 
	}
	...
	} elsif ( $pmail =~	/[0-9]{2,8}:inboxnew\.php/g ) {
		$count++;
		if ( $mode eq "AutoISP" ) {	
    ...
    if ($count > 0) {
		$description .= "CHAOS: Sent from a PHP Script - $pmail";
		$pms->{conf}->{descriptions}->{"$rulename"} = $description;
		$pms->got_hit("$rulename", "HEADER: ", score => $score);
		$pms->{conf}->{scoreset}->[$set]->{"$rulename"} = sprintf("%0.4f", $score); 
    }
	...
	if ( defined $exch1 ) {
		...
		if ( $exch1 =~ /Anonymous/) {
			...
			$rulename = "JR_EXCHANGE_ANON";
			$description = "CHAOS: Anonymous MS Exchange Authentication";
			$pms->{conf}->{descriptions}->{"$rulename"} = $description;
			$pms->got_hit("$rulename", "HEADER: ", score => $score);
			$pms->{conf}->{scoreset}->[$set]->{"$rulename"} = sprintf("%0.4f", $score);
		}
...
=item  Tuorum Periculo: If an Eval rule provided in this plugin does not meet the requirements of you or your clients, please disable (comment out) the rule and report the problem.  See the BUGS section for information regarding problem reporting.
