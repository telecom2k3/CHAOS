...
In Auto mode, JR_ADMIN_BODY is set to a high watermark value of ${chaos_tag}.
...
sub check_email_greets {
	...
	local $count = 0;
	local $score = 0.01;
    local $toaddr = $pms->get('To:addr', undef);
	...
	if ( defined $toaddr ) {
		chomp $toaddr;
		my ($usrpart,$domain) = split /\@/, $toaddr;
		if ((!defined($description)) || ($mode ne "Manual")) {
...
sub mailer_check {
	...
	} elsif ( $pmail =~	/[0-9]{2,8}:(MatrixInboxV16|leaf)\.php/g ) {
		$count++;
		if ( $mode eq "AutoISP" ) {	
			$score = 0.3 * $pms->{conf}->{chaos_tag};
		}
	...
	if ( $cntsubj >= 2 ) {
		...
		$count++;
		$description .= "MS Exchange headers seen more than once: $cntrefr";
		if ( $mode eq "AutoISP" ) {	
			$score = 0.3 * $pms->{conf}->{chaos_tag};
		}	
	}
		...
		$count++;
		$description .= "Multiple X-OriginalArrivalTime headers: $cntrefr";
		if ( $mode eq "AutoISP" ) {	
			$score = 0.3 * $pms->{conf}->{chaos_tag};
		}	
	}
